...
SET @has_telefono := (
  ...
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'la_stg_prospectos'
    AND COLUMN_NAME = 'telefono_extraido'
);

SET @sql_telefono := IF(
  @has_telefono = 0,
  'ALTER TABLE la_stg_prospectos ADD COLUMN telefono_extraido VARCHAR(50) NULL AFTER direccion_extraida',
  'SELECT 1'
);
...
SET @sql_whatsapp := IF(
  @has_whatsapp = 0,
  'ALTER TABLE la_stg_prospectos ADD COLUMN whatsapp_extraido VARCHAR(50) NULL AFTER telefono_extraido',
  'SELECT 1'
);
